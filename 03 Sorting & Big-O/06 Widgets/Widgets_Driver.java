//name:Shivani Puli    date:11/03/18
...
public class Widgets_Driver
{
   ...
   public static Comparable[] input(String filename) throws Exception
   {
      ...
      Comparable[] a=new Comparable[numberOfWidgets];
      for(int x=0;x<a.length;x++)
         a[x]=new Widget(f.nextInt(),f.nextInt());
      return a;
   }
...
class Widget implements Comparable<Widget>
{
   ...
   {
      if(myCubits<w.myCubits)
         return -1;
      else if(myCubits>w.myCubits)
         return 1;
      else if(myHands<w.myHands)
         return -1;
      else if(myHands>w.myHands)
         return 1;
      else
