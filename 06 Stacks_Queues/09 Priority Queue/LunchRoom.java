...
public class LunchRoom
{
   ...
   public static void main(String[] args)
   {
      ...
      ArrayList<Integer> soph=new ArrayList();
      ...
      for(int x=0; x<TIME; x++)
      {
         ...
         if(rand==0)
         {
            rand=(int)(Math.random()*4)+9;
            queue.add(new Customer(rand,x));
         }
         if(!queue.isEmpty())
         {
            if(queue.peek().serve())
            {
               int type=queue.peek().getGrade();
               if(type==9)
                  fresh.add(queue.poll().waitTime());
               else if(type==10)
                  soph.add(queue.poll().waitTime());
               else if(type==11)
                  jun.add(queue.poll().waitTime());
               else
                  sen.add(queue.poll().waitTime());
            }
         }
         waits(queue);
         display(x,queue);
      ...
      /* report the data  */
      System.out.println("Senior\t\t" + sen.size() + "\t\t" + Collections.max(sen) + "\t\t" + average(sen));
      System.out.println("Junior\t\t" + jun.size() + "\t\t" + Collections.max(jun) + "\t\t" + average(jun));
      System.out.println("Sophomore\t\t" + soph.size() + "\t\t" + Collections.max(soph) + "\t\t" + average(soph));
      System.out.println("Freshmen\t\t" + fresh.size() + "\t\t" + Collections.max(fresh) + "\t\t" + average(fresh));
   
   
   }
   
   public static double average(ArrayList<Integer> f)
   {
      double avg=0;
      for(int x: f)
         avg+=x;
   ...
   public static void display(int t, PriorityQueue<Customer> q)
   {
      ...
      if(!q.isEmpty())
      {
         Iterator<Customer> tr=q.iterator();
         while(tr.hasNext())
         {
            Customer f=tr.next();
            print+=f.toString() + ", ";
   ...
   public static void waits(PriorityQueue<Customer> q)
   {
      Iterator<Customer> t=q.iterator();
      while(t.hasNext())
         t.next().setWait();
   }
}
...
class Customer implements Comparable<Customer>
{
   ...
   private int wait=0;
   private int name;
   private int serveTime=(int)(Math.random()*6)+2;
   ...
   public int compareTo(Customer f)
   {
      int comp= f.getGrade()-grade;
      return comp;
   }
